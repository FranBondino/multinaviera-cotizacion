...
const MSC_CONFIG = {
  endpoint: process.env.MSC_API_ENDPOINT || 'https://portal.api.msc.com/dpo/ovconnectsch/routes/v1/sailingRoutes',
  subscriptionKey: process.env.MSC_SUBSCRIPTION_KEY,
  apiKey: process.env.MSC_API_KEY,
  userAgent: process.env.MSC_USER_AGENT || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
};
...
export async function fetchMscLiveSchedule(polUnlocode, podUnlocode, options = {}) {
  try {
    ...
    const url = `${MSC_CONFIG.endpoint}?${params.toString()}`;

    const headers = {
      'Accept': 'application/json',
      'User-Agent': MSC_CONFIG.userAgent
    };

    if (MSC_CONFIG.subscriptionKey) {
      headers['Ocp-Apim-Subscription-Key'] = MSC_CONFIG.subscriptionKey;
    }
    if (MSC_CONFIG.apiKey) {
      headers['x-api-key'] = MSC_CONFIG.apiKey;
    }
